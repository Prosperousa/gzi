...
$(document).ready(function(){
	//动态添加大转盘的奖品与奖品区域背景颜色
	turnplate.restaraunts = [ "一等奖", "二等奖", "三等奖", "一等奖", "二等奖", "三等奖"];
	turnplate.colors = ["#FFFFFF", "#EFD691", "#FFFFFF", "#EFD691","#FFFFFF", "#EFD691"];
	//turnplate.fontcolors = ["#CB0030", "#ssFFFFFF", "#CB0030", "#FFFFFF","#CB0030", "#FFFFFF"];
...
function JieShu(){
	 // 判断结束时间
    let today=new Date();
    let jieshu=today.setDate(today.getDate()+1);
    let zj=document.querySelector(".zj-main");
    let wzj=document.querySelector("xxcy-main");
    if (today <= jieshu) {
    	alert(1);
    	zj.style.display="block";
    }else{
    	wzj.style.display="block";
    	alert(2);
    }
}
...
function drawRouletteWheel() {    
  ...
  if (canvas.getContext) {
	  ...
	  for(var i = 0; i < turnplate.restaraunts.length; i++) {       
		  ...
		  ctx.arc(270, 270, turnplate.outsideRadius, angle, angle + arc, false);    
		  ctx.arc(270, 270, turnplate.insideRadius, angle + arc, angle, true);
		  ctx.stroke();  
		  ctx.fill();
		  //锁画布(为了保存之前的画布状态)
		  ctx.save();   
